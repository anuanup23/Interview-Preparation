...
class NodeValue{
    ...
    NodeValue(int min_val, int max_val, int size){
    ...
    NodeValue largestBstHelper(Node * root){
        ...
        if(root->data > left.max_val && root->data < right.min_val)
            return NodeValue(min(root->data,left.min_val), max(root->data,right.max_val),left.size + right.size + 1);
